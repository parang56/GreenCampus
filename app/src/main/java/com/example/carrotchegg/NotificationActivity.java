...
public class NotificationActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Query query = FirebaseFirestore.getInstance()
                .collection("userData").document(uid).collection("notification")
        ...
        adapter = new FirestoreRecyclerAdapter<UserData, NotificationHolder>(options) {

            @Override
            public void onBindViewHolder(NotificationHolder holder, int position, UserData model) {
                holder.bind(model, NotificationActivity.this);
            }

            @Override
            public NotificationHolder onCreateViewHolder(ViewGroup group, int i) {
                View view = LayoutInflater.from(group.getContext())
                        .inflate(R.layout.post_form2, group, false);
